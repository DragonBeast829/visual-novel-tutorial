...
public class HistoryLogManager : MonoBehaviour {
    ...
    private const float TEXT_MAX_SCALE = 50f;
    private const string NAMETEXT_NAME = "Name Text";
    ...
    private void CreateLog(HistoryState state) {
        ...
        if (state.dialogue.currentSpeaker == string.Empty) {
            ...
            log.nameFontSize = Mathf.Clamp(TEXT_DEFAULT_SCALE * state.dialogue.speakerScale, 0, TEXT_MAX_SCALE);
